  ...
  return <AnimatePresence>
      {isOpen && <>
        ...
        y: 20
            ...
            <div className="p-2">
              {shortcuts.map((shortcut, index) => <div key={index} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors">

                  <span className="text-sm text-gray-600">
                    {shortcut.description}
                  </span>
                  <div className="flex gap-1">
                    {shortcut.keys.map((key) => <kbd key={key} className="px-2 py-1 bg-white border border-gray-200 rounded-md text-xs font-semibold text-gray-500 shadow-sm min-w-[24px] text-center">
