...
export function QuestionItem({
  ...
  const categoryConfig = CATEGORY_CONFIG[question.category] || CATEGORY_CONFIG.all;
    ...
    borderRight: isHovered ? '4px solid #444444' : '4px solid transparent'
      ...
      <div className="flex-1 flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-4 min-w-0">
        {showCheckbox && onCheckboxChange && <div className="transition-opacity duration-200 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
            ...
            <Checkbox checked={isChecked} onChange={onCheckboxChange} id={`question-${question.id}`} />
        ...
        <div className="flex-1 min-w-0 cursor-pointer">
          <div className="flex items-start gap-2 sm:gap-3">
            <div className="flex-1 min-w-0">
              ...
              {/* Text - No tooltip, hidden on very small screens */}
            ...
            <div className="relative flex-shrink-0">
              <div className={`flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 rounded-full transition-all duration-200 cursor-pointer ${isPublished ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`} onMouseEnter={() => setShowStatusTooltip(true)} onMouseLeave={() => setShowStatusTooltip(false)}>
