...
export function Input({
  ...
  return <div className={`${fullWidth ? 'w-full' : ''} mb-4`}>
      {label && <label htmlFor={inputId} className="block text-xs font-semibold text-gray-600 uppercase mb-1.5 tracking-wide ml-0.5">

          {label} {required && <span className="text-red-500">*</span>}
        </label>}
      <div className="relative group">
        {(icon || variant === 'search') && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-teal-500 transition-colors">
            {variant === 'search' ? <Search className="h-4 w-4" /> : icon}
          </div>}
        <motion.input id={inputId} whileFocus={{
        scale: 1.01
      }} transition={{
        type: 'spring',
        stiffness: 300,
